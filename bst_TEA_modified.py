...
if TYPE_CHECKING: from biosteam._system import System
import qsdsan as qs
...
class TEA:
    ...
    **Abstract methods**
    ...
    depreciation_schedules: dict[tuple(str, int), NDArray[np.float64]] = {
    ...
    def __init__(self, system: System, IRR: float, duration: tuple[int, int], 
                 depreciation: str|NDArray[np.float64], income_tax: float,
    ...
    @property
    def depreciation(self) -> str|NDArray[np.float64]:
        """
        Depreciation schedule array or a string with format '{schedule}{years}', 
        where years is the number of years until the property value is zero 
        and schedule is one of the following: 'MACRS' (Modified Accelerated Cost Recovery System), 
    ...
    def utility_cost(self) -> float:
        ...
        result = []
        for i in qs.Flowsheet.flowsheet.default.unit:
            PandH = i.utility_cost*self.operating_days*24 + sum([k for k in i.add_OPEX.values()])*self.operating_days*24
            result.append(PandH)
        return sum(result)
    @property
    ...
    @property
    def cashflow_array(self) -> NDArray[np.float64]:
        """Cash flows by year."""
        return sum(self._net_earnings_and_nontaxable_cashflow_arrays())
    
    @property
    def net_earnings_array(self) -> NDArray[np.float64]:
